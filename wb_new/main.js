var SERVER = window.location.protocol + "//" + window.location.host + '/jayasri_rework/';
var API = window.location.protocol + "//" + window.location.host +  '/jayasri_rework/API/';
...
$('#save_hasil').on('click',function(){  
    if     ($('#taraOCR').val()            < 0)err('Timbang 1 Kosong.');
    else if($('#plate_recognize').val() === ''
    && $('#plat_correct').val()         === '')err('Nopol Harus Diisi.');
    else if($('#pengemudi').val()       === '')err('Sopir Harus Diisi.');
    else{
        $(this).html('Process..');
        $(this).attr('disabled','disabled');
        save_hasil();
