...
//				$this->maxX = max($this->maxX, $x+$w);
		...
		*/	
		protected function drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background)
		{
			...
			if ($in_TL == NULL || $in_TL[0]<=0 || $in_TL[1]<=0) $in_TL = null;
			if ($in_TR == NULL || $in_TR[0]<=0 || $in_TR[1]<=0) $in_TR = null;
			if ($in_BR == NULL || $in_BR[0]<=0 || $in_BR[1]<=0) $in_BR = null;
			if ($in_BL == NULL || $in_BL[0]<=0 || $in_BL[1]<=0) $in_BL = null;
